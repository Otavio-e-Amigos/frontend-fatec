...
const defaultTableTest = new DefaultTableModel(
	[
		...
		[
			"data04",
			{ value: 2, display: <p className="text-amber-500 cursor-help">2</p> },
			"data06",
		],
		["data07", 4, "data09"],
		[
			...
			6,
			{
				...
				display: (
					<span className="flex flex-row gap-2 items-center">
						<img src="/favicon.ico" /> <p>data12</p>
					</span>
				),
			},
		],
		[
			...
			1,
			{
				...
				display: (
					<span className="flex flex-row gap-2 items-center">
						<img src="/favicon.ico" /> <p>data13</p>
					</span>
				),
			},
		],
...
function StepBadge({
	...
	const concludedColor = concluded
		? "bg-hint-success"
		: "bg-hint-success-inactive";
	const activeColor = concluded
		? "bg-hint-success"
		: active
			? "bg-hint-success"
			: "bg-hint-success-inactive";
	return (
		<div className="flex flex-col">
			<div className="flex flex-row items-center gap-2">
				<div
					className={`rounded-full ${activeColor} size-15 flex items-center justify-center`}
				>
					{step}
				</div>
				{!final && (
					<span
						className={`max-h-1.5 size-15 ${concludedColor} rounded-full`}
					/>
				)}
			</div>
			<p className="overflow-hidden wrap-break-word flex-wrap w-15">
				{description}
			</p>
		</div>
	);
		...
		<main>
			...
			<section className="mt-5 flex flex-col mx-5">
				...
				<section className="flex flex-col gap-3 w-fit">
					...
					<section className="flex flex-col gap-1 w-fit">
						<StatusBadge value={"Completed"} status={"green"} fill />
						<StatusBadge value={"Pending"} status={"yellow"} fill />
						<StatusBadge value={"Not Completed"} status={"red"} fill />
						<StatusBadge value={"Unknown"} status={"gray"} fill />
					</section>
				</section>
				...
				{/* TODO text need to fit entire words or descriptions inside badge */}
				{/*<p>as components</p>
				<section className="flex flex-row items-start gap-2 mb-5">
					<StepBadge
						...
						concluded
					/>
					<StepBadge step={2} description={"Adicionar Disciplinas"} active />
					<StepBadge step={3} description={"Configurar Grades"} final />
				</section>*/}
